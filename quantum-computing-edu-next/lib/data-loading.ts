...
const loadGatesAndCircuits = cache((): [GateMap, CircuitMap] => {
  const gate_data = fs
    .readdirSync(gateDataDir)
    .filter(file => file.endsWith(".json"))
    .toSorted()
    .map(file => loadDataFile(file, gateDataDir) as GateData);

  const circuit_data = fs
    .readdirSync(circuitDataDir)
    .filter(file => file.endsWith(".json"))
    .toSorted()
    .map(file => loadDataFile(file, circuitDataDir) as QuantumCircuitData);

  const gate_map = new Map<string, Gate>();
  const circuit_map = new Map<string, QuantumCircuit>();
  ...
  while (gate_data.length > 0 || circuit_data.length > 0) {
    ...
    while (i < gate_data.length) {
      try {
        ...
        if (
          error instanceof CircuitNotFoundError ||
          ...
          error instanceof GateParsingError ||
          error instanceof CircuitParsingError
        ) {
    ...
    if (gate_data.length + circuit_data.length == prev_pending_count) {
      throw new Error(
        "Some gates/circuits could not be parsed due to missing circuit/gate dependencies. Check gates [" +
...
export { circuitDataDir, gateDataDir, loadGatesAndCircuits };
